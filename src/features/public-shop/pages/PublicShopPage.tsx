    ...
    return (
        <PublicLayout theme={shopData.theme}>
            ...
            <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-100 px-4 py-3">
                <div className="max-w-4xl mx-auto flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        {shopData.logoUrl ? (
                            <img src={shopData.logoUrl} alt={shopData.name} className="w-10 h-10 rounded-full object-cover shadow-sm border border-slate-100" />
                        ) : (
                            <div className="w-10 h-10 rounded-full bg-shop-primary flex items-center justify-center text-white font-bold shadow-sm">
                                {shopData.name.substring(0, 2).toUpperCase()}
                            </div>
                        )}
                        <h1 className="font-bold text-lg text-slate-900 tracking-tight">{shopData.name}</h1>
                    </div>
                    <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 border-2 border-shop-primary cursor-pointer hover:bg-slate-200 transition-colors shadow-sm">
            ...
            {shopData.layout.showHero && (
                <div className="md:max-w-4xl md:mx-auto md:px-4 md:pt-4">
                    <div className="relative h-64 md:h-80 bg-slate-900 overflow-hidden md:rounded-3xl shadow-lg">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                            <h2 className="text-3xl md:text-5xl font-extrabold text-white mb-3 drop-shadow-2xl">
                                {shopData.layout.heroTitle}
                            </h2>
                            {shopData.layout.heroSubtitle && (
                                <p className="text-lg md:text-xl text-slate-200 drop-shadow-xl font-medium">
                                    {shopData.layout.heroSubtitle}
                                </p>
                            )}
                        </div>
            ...
            <div className="max-w-4xl mx-auto p-4 space-y-8">
                ...
                <section className="bg-white rounded-2xl p-6 shadow-md border border-slate-100">
                    <h3 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2">
                        <Icon name="phone" className="w-5 h-5 text-shop-primary" />
                        Contato e Localização
                    </h3>
                    ...
                    <div className="space-y-4">
                        ...
                        {shopData.address && (
                            <button
                                onClick={() => {
                                    const searchQuery = `${shopData.address}${shopData.city ? `, ${shopData.city}` : ''}${shopData.state ? ` - ${shopData.state}` : ''}`;
                                    window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(searchQuery)}`, '_blank');
                                }}
                                className="flex items-start gap-4 p-4 rounded-xl hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all group w-full text-left border border-slate-200 shadow-sm"
                            >
                                <div className="min-w-0 flex-1">
                                    <p className="font-bold text-slate-900 group-hover:text-shop-primary transition-colors">Endereço</p>
                                    <p className="text-slate-600 text-sm">{shopData.address}</p>
                                </div>
                                <Icon name="right" className="w-4 h-4 text-slate-400 ml-auto self-center group-hover:translate-x-1 transition-transform" />
                            </button>
                        )}
                        ...
                        <div className="flex flex-wrap gap-3 pt-2">
                            {shopData.phone && (
                                ...
                                >
                                    <Icon name="whatsapp" className="w-5 h-5 text-green-500" />
                                    WhatsApp
                                </a>
                            )}
                            ...
                            {shopData.website && (
                                ...
                                >
                                    <Icon name="globe" className="w-5 h-5 text-red-500" />
                                    Site
                                </a>
                            )}
                        </div>
